define ['underscore', 'backbone', 'util/util', 'model/user', 'model/loading', 'model/spreadsheet_list', 'view/navbar_view', 'view/loading_view', 'text!template/project_list.html', 'view/tab_controller', 'view/project_view'], (_, Backbone, Util, User, Loading, SpreadsheetList, NavbarView, LoadingView, ProjectListTemplate, TabController, ProjectView) ->
  class ProjectListView extends Backbone.View
    ...
    tabSelected: ->
      unless event.target.id == @selected || event.target.className == "x_icon" || event.target.className == "tab_default_x"
        ...
        @projectView.updateProjectView(@selected[1])
